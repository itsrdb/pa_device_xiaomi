...
Light::Light() {
    ...
    mLights.emplace(Type::BATTERY, std::bind(&Light::handleBattery, this, std::placeholders::_1, 1));
    ...
    auto getScaledDutyPercent = [](int brightness) -> std::string {
        ...
        for (int i = 0; i <= kRampSteps; i++) {
            ...
            if (i <= kRampSteps / 2) {
                ...
                output += std::to_string((i - kRampSteps / 2) * 300 * brightness /
...
void Light::handleNotification(const LightState& state, size_t index) {
    ...
    auto getScaledDutyPercent = [](int brightness) -> std::string {
        std::string output;
        for (int i = 0; i <= kRampSteps; i++) { //Modified this
            if (i != 0) {
                output += ",";
            }
    ...
    if (onMs > 0 && offMs > 0) {
        uint32_t pauseLo, pauseHi, stepDuration;
        if (kRampMaxStepDurationMs * kRampSteps > onMs) { //Modified this
